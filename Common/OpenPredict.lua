--[[
  ...
  What's changed?
    Complete rewrite
    predictInfo:hCollision(minCollisionUnits, team) (Added team argument, possible to use value such as TEAM_ALLY + TEAM_NEUTRAL)
    predictInfo:mCollision(minCollisionUnits, team) (Same as above)
    predictInfo:pCollision() (Checks for projectile interception - spells like Unbreakable and Wind Wall)
    predictInfo:checkCollision(unit) -- (Standard collision check against units - useful for things like Zyra plants)
    OpenPredictExportAll(environment) (Exports non-standard api to environment)
]]
